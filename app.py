...
st.set_page_config(
    page_title="International Registration ETL",
    page_icon="🌏",
    layout="wide"
)
...
st.markdown(
    '<div class="main-title">🌏 INTERNATIONAL REGISTRATION ETL SYSTEM</div>',
    unsafe_allow_html=True
)

st.markdown(
    '<div class="sub-title">'
    'ระบบ ETL ข้อมูลผู้สมัครนักศึกษาต่างชาติ'
    '</div>',
    unsafe_allow_html=True
)
...
def run_etl(uploaded_file):
    ...
    # 3. Convert Date
    date_columns = ["Birthdate", "Submitted Date"]
    ...
    if "University" in df.columns:
        university_mapping = {
            "china weat normal university": "China West Normal University"
        }
        df["University"] = (
            df["University"].replace(university_mapping).str.title()
        )
    ...
    # 11. Create Major1 from Major
    # ตัด Doctor of / Master of และตัด Type / Plan พร้อมค่าที่ตามหลัง
    if "Major" in df.columns:
        df["Major1"] = (
            df["Major"].astype("string")
            .str.replace(
                r"\b(?:Doctor|Master)\s+of\b",
...
uploaded_file = st.file_uploader(
    "เลือกไฟล์ Excel ที่ต้องการประมวลผล",
    type=["xlsx", "xls"],
    help="รองรับไฟล์ Excel .xlsx และ .xls"
)
...
if "etl_df" in st.session_state:
    ...
    with col1:
        st.metric("ข้อมูลก่อน ETL", f"{summary['rows_before']:,}")
    with col2:
        st.metric("ข้อมูลหลัง ETL", f"{summary['rows_after']:,}")
    with col3:
        st.metric("ข้อมูลซ้ำที่ลบ", f"{summary['duplicates_removed']:,}")
    with col4:
    ...
    source_name = st.session_state.get(
        "source_name", "international_registration.xlsx"
    )
    output_name = Path(source_name).stem + "_ETL.xlsx"

    st.download_button(
        label="⬇️ ดาวน์โหลดข้อมูล ETL (.xlsx)",
        data=excel_data,
